...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($name)) {
        ...
        try {
            ...
            $error_message = 'Error updating supplier. Please try again.';
...
<div class="d-flex justify-content-between align-items-center mb-4">
    <div>
        ...
        <p class="text-muted mb-0" style="font-size: 13px;">Modify sourcing vendor profiles and contact information.</p>
    </div>
    <div>
        ...
        <div class="card card-custom border-0 shadow-sm p-4 mb-4">
            <form method="POST" class="row g-4">
                ...
                <div class="col-12">
                    ...
                    <input type="text" id="name" name="name" class="form-control p-3 rounded-3" required value="<?php echo htmlspecialchars($supplier['name']); ?>" placeholder="e.g. Acme Corporation">
                </div>

                <!-- Email -->
                <div class="col-md-6">
                    <label for="email" class="form-label fw-semibold text-secondary">Email Address</label>
                    <input type="email" id="email" name="email" class="form-control p-3 rounded-3" value="<?php echo htmlspecialchars($supplier['email'] ?? ''); ?>" placeholder="e.g. contact@acme.com">
                </div>

                <!-- Phone -->
                <div class="col-md-6">
                    <label for="phone" class="form-label fw-semibold text-secondary">Phone Number</label>
                    <input type="text" id="phone" name="phone" class="form-control p-3 rounded-3" value="<?php echo htmlspecialchars($supplier['phone'] ?? ''); ?>" placeholder="e.g. +1 (555) 019-2834">
                </div>

                <!-- Address -->
                <div class="col-12">
                    <label for="address" class="form-label fw-semibold text-secondary">Corporate Address</label>
                    <textarea id="address" name="address" class="form-control p-3 rounded-3" rows="3" placeholder="Enter corporate physical address..."><?php echo htmlspecialchars($supplier['address'] ?? ''); ?></textarea>
                </div>

                <!-- Submit -->
                <div class="col-12 pt-2">
                    <button type="submit" class="btn btn-primary w-100 py-3 rounded-3 fw-bold"><i class="bi bi-check-circle"></i> Save Vendor Changes</button>
                </div>
            </form>
        </div>
        <?php endif; ?>
